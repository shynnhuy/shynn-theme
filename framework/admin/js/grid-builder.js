	...
	$usof.field[ 'grid_builder' ] = {
		...
		_updateElementPlaceholder: function( $elm, id, values ) {
			...
			$.each( [ 'left', 'top', 'right', 'bottom' ], function( _, dir ) {
				if ( values[ 'design_options' ] && values[ 'design_options' ][ 'position_' + dir + '_default' ] !== undefined && values[ 'design_options' ][ 'position_' + dir + '_default' ] !== '' ) {
					isAbs = true;
				}
			} );
		...
		_optionChanged: function( field ) {
			...
			if ( fieldId === 'fixed' ) {
				var posValues = [
					'position_bottom_default',
					'position_left_default',
					'position_right_default',
					'position_top_default'
				];
				$.each( this.value[ state ].layout[ 'middle_center' ], function( _, elmId ) {
					if ( ! this.value.data[ elmId ].design_options ) {
						this.value.data[ elmId ].design_options = {};
					}
					...
					for ( i = 0; i < posValues.length; i ++ ) {
						isAbs = ( isAbs || ( desOpt[ posValues[ i ] ] != '' ) );
					}
