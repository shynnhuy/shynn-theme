...
foreach ( $field['options'] as $key => $img_url ) {
	...
	$output .= '<label for="' . $id . '_' . $key . '"><img src="' . trailingslashit( $us_template_directory_uri ) . $img_url . '.png" srcset="' . trailingslashit( $us_template_directory_uri ) . $img_url . '-2x.png 2x" alt="' . $key . '"></label>';
	$output .= '</li>';
}
$output .= '</ul>';
